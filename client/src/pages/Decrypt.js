  ...
  const handleDecrypt = async () => {
    ...
    try {
      ...
      if (['CTR', 'CFB', 'OFB', 'XTS', 'GCM'].includes(mode)) {
        ...
        // Add nonce for CTR mode
        if (mode === 'CTR' && nonce) {
          payload.nonce = nonce;
        }
        
        // Add IV for CFB, OFB, GCM, XTS modes
        if ((mode === 'CFB' || mode === 'OFB' || mode === 'GCM' || mode === 'XTS') && iv) {
          payload.iv = iv;
        }
          ...
          >
            ...
            <div className="mb-6">
              <p className="text-gray-700 dark:text-gray-300 mb-4">
                To protect your decryption history, please go to the <strong>History page</strong> and set a password first. This will encrypt and secure all your encryption/decryption records.
              </p>
            ...
            <div className="glass rounded-2xl p-6 border-clean">
              ...
              <motion.button
                ...
                disabled={
                  ...
                  cleanedCiphertext.length === 0 || 
                  cleanedKey.length !== parseInt(keySize)/4 || 
                  (mode === 'CBC' && cleanedIV.length !== 32) ||
                  (mode === 'CTR' && nonce.length !== 24) ||
                  ((mode === 'CFB' || mode === 'OFB' || mode === 'GCM' || mode === 'XTS') && cleanedIV.length === 0)
                }
              ...
              {/* Validation Status */}
              {inputMode === 'text' && cleanedCiphertext.length > 0 && cleanedKey.length === parseInt(keySize)/4 && 
               (mode === 'ECB' || 
                (mode === 'CBC' && cleanedIV.length === 32) ||
                (mode === 'CTR' && nonce.length === 24) ||
                ((mode === 'CFB' || mode === 'OFB' || mode === 'GCM' || mode === 'XTS') && cleanedIV.length > 0)) && 
